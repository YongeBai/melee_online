...
try {
  ...
  if(tournament){
    ...
    if(menuHandoff)module._portTournamentInitializeMenu();else module._portTournamentInitializeCostumes(kind,opponentKind,stageSpec.kind,costumeIndex,opponentCostumeIndex);report.rules=Array.from({length:12},(_,i)=>module._portTournamentRead(i,0));
    if(withHud){
  ...
  if(tournament||live||params.has('step')){
    ...
    if(live){
      const controls=document.createElement('p');controls.textContent='Native port development fixture — arrows: move; X: jump; Z: attack; S: special; C: grab; Shift: shield. Enter/Escape: Start. Native menus, audio and full scene ownership remain incomplete.';canvas.before(controls);
      ...
      await new Promise((resolve,reject)=>{
        globalThis.nativeLive=startNativeLive(module,preview,[object,opponent].filter(Boolean),{step,unlockInput:!menuHandoff,readMatch:withHud?()=>({pause:Array.from({length:5},(_,i)=>module._portTournamentPauseRead(i)),clock:[12,13,14].map(i=>module._portTournamentRead(i,0)),...(menuHandoff?{intro:{mask:module._portTournamentRead(23,0),gate:module._portTournamentRead(17,0),blocked:[0,1].map(p=>module._portTournamentRead(24,p))}}:{})}):null,resolveObjects:()=>[object,opponent].filter(Boolean),frameLimit:limit,inputProvider:params.has('workload')?combatWorkload:null,
