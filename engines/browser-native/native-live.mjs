...
export function startNativeLive(module,preview,objects,{frameLimit=0,onProgress=()=>{},onComplete=()=>{},onError=()=>{},onDraw=()=>{},step=()=>module._portRuntimeStep(),inputProvider=null,browserInput=null,resolveObjects=null,readMatch=null,shouldFinish=()=>false,unlockInput=true,network=null,rollback=null}={}) {
  ...
  function finish(){completionReason="match-end";stop();onComplete(snapshot());}
  function frame(now){
    if(stopped)return;
    try {
      rollback?.reconcile();
      ...
      let advanced=0;for(let index=0;index<steps;index++) {
        ...
        const tracked=rollback?samples[rollback.seat]:samples[0];if(stateChanges.length<256&&stateChanges.at(-1)?.state!==final[0][0])stateChanges.push({frame:frames,state:final[0][0],x:final[0][4],y:final[0][5],buttons:tracked[0]});
  ...
  if(!rollback)network?.begin('match');raf=requestAnimationFrame(frame);
