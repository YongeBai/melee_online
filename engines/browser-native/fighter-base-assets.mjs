...
// plus typed Article tables, Fox's scalar extra record, and Purin's
...
export function convertFighterBase(input,name,{motionSpec,attributeSpec,partCount,costumes}) {
  ...
  if(fighterArticleProfiles[code]) {
    if(!a.relocations.has(source+0x48))throw Error('Missing fighter item table');
    ...
    const table=d.getUint32(source+0x48),entries=new Uint8Array(fighterArticleProfiles[code].slots*4),v=new DataView(entries.buffer),start=append(entries);
    if(table+entries.length>a.dataSize)throw Error('Fighter item table bounds');
