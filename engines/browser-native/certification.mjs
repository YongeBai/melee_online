...
if(mode==='calibration'){
 ...
 const runtime=await createSnapshotRuntime(create,bytes,{dirtyManifest:dirty?.manifest,onNativeMusic:r=>audio.request(r),onNativeAudioMode:()=>true}),module=runtime.module,cache=createPresentationCache({uniformBuffer:params.get('uniformbuffer')!=='0',exactState:params.get('exactstate')!=='0',compactShaderKey:params.get('shaderkey')!=='0',compactTextureKey:params.get('texturekey')!=='0',profileDraw:params.get('drawtiming')==='1',submissionOptimized:params.get('drawopt')!=='0',packedState:params.get('packedstate')!=='0',reuseImmediate:params.get('immediatereuse')==='0'?false:params.get('immediatereuse')==='1'?true:undefined,cacheImmediateViews:params.get('immediateviews')!=='0',cacheTextureView:params.get('textureview')!=='0'});
 ...
 const boot=await runNativeConstructor({module,presentationCache:cache,canvas:picture,params:{tournament:1,hud:1,damagehud:1,intro:1,stagecallbacks:1,render:1,rendersteps:1,map:params.get('map')??'battlefield',character:params.get('character')??'Fc',opponent:params.get('opponent')??'Fx',gpuerrors:'deferred',...(params.get('map')==='fountain'?{fountaincosmetics:'off',fountainscenery:'off'}:{})},onMatchBoundary:async boundary=>{
  ...
  const frameOracleCache=frameOracleEnabled?createPresentationCache({uniformBuffer:false,exactState:false,submissionOptimized:false,packedState:false,reuseImmediate:false,compactTextureKey:false}):null,frameOracle={enabled:frameOracleEnabled,frames:0,comparedBytes:0,differentBytes:0,cameraMismatches:0};
  ...
  const snapshotTimings=[],kernelTimings=[];let trackTimings=false;
  const sparse=!!dirty&&params.get('sparserestore')!=='0';
  const store=createPagedWasmCheckpointStore({...runtime,host:audio,maxBytes:2*1024**3,sparse,auditSparse:params.get('snapshotaudit')==='1',verifyImmutableTable,onTiming:r=>{if(trackTimings)snapshotTimings.push(r);}});
  const initial=store.capture(),packets=[],workload={hitlagFrames:0,damageFrames:0,attackFrames:0,stockChanges:0};
  const corrections=[],sim=[],replaySim=[],advance=[],copy=[],capture=[],rebind=[],draw=[],dispose=[],restore=[],total=[],times=[],cameras=[],drawStages=[],glCalls=[],uniformFrames=[],frameTimeline=[],profileTimeline=params.get('timeline')==='1';
  ...
  for(let f=0;f<frames;f++){packets[f]=combatWorkload(f,previous).map(pad=>({pad:completeNativeSample(pad),tap:1}));audio.beginFrame(f);rawStep(packets[f]);const current=boundary.readPlayers();workload.hitlagFrames+=current.some(p=>p[14]>0);workload.damageFrames+=current.some(p=>p[13]>0);workload.attackFrames+=current.some(p=>p[0]>=44&&p[0]<=69);workload.stockChanges+=current.filter((p,i)=>p[18]!==previous[i][18]).length;previous=current;}
  const reference=store.capture(),referenceHash=await store.hash(reference),referenceJournal=audio.capture(),expectedPlayers=boundary.readPlayers();store.restore(initial);
  ...
  const measuredStep=(inputs,{replay=false,frame=forward}={})=>{audio.beginFrame(frame);const t=performance.now();rawStep(inputs);if(measuring)(replay?replaySim:sim).push(performance.now()-t);};
  const camera=d=>({resolution:d.resolution,eye:d.eye,interest:d.interest,fov:d.fov,aspect:d.aspect,near:d.near,far:d.far});
  function drawFrame(preview){const t=performance.now();lastDraw=preview.draw();if(lastDraw.materialDraws.uniformBufferFrame)uniformFrames.push(lastDraw.materialDraws.uniformBufferFrame);if(lastDraw.materialDraws.drawTiming)drawStages.push(lastDraw.materialDraws.drawTiming);if(lastDraw.materialDraws.exactGL)glCalls.push(lastDraw.materialDraws.exactGL);draw.push(performance.now()-t);observer.request();times.push(performance.now());cameras.push(camera(lastDraw));}
  ...
  async function finish(){trackTimings=false;stopped=true;finishTime=performance.now();const replicaMeasured=replica?.metrics()??null;const observation=await observer.stop();persistent?.validateGpu();persistent?.dispose();persistent=null;
   const actualPlayers=boundary.readPlayers(),audioJournalMatchesReference=JSON.stringify(audio.capture())===JSON.stringify(referenceJournal),final=store.capture(),finalHash=await store.hash(final);let oracle,oracleCamera,cachedCamera,cachedPixels,freshPixels;
   ...
   if(!audioJournalMatchesReference||audio.snapshot().presented)failures.push('Audio journal/emission mismatch');
   ...
   globalThis.certificationReport={diagnosticPassed:!failures.length,performanceCertified:false,failures,mode,seat,frames,elapsedMs:elapsed,simulationFps:1000*forward/elapsed,drawSubmissions:draw.length,submissionCadence:submission,observation,workload,actualWorkload,referenceHash,finalHash,stateMatches,measuredPlayersMatchReference:JSON.stringify(actualPlayers)===JSON.stringify(expectedPlayers),expectedPlayers,actualPlayers,pixelOracle:{differentBytes,comparedBytes:cachedPixels.length},frameOracle,camera:{sameAsFresh:sameCamera,corrected:cachedCamera,first:cameras[0],last:cameras.at(-1),fresh:oracleCamera,validatedEveryDraw:true},frameTimeline,costs:{simulation:distribution(sim),correction:distribution(corrections),replaySimulation:distribution(replaySim),advanceIncludingCorrections:distribution(advance),presentationCopy:distribution(copy),presentationCapture:distribution(capture),rendererRebind:distribution(rebind),drawSubmission:distribution(draw),rendererDispose:distribution(dispose),presentationRestore:distribution(restore),forwardCallback:distribution(total)},uniformBufferMeasured:uniformFrames.reduce((sum,r)=>{sum.frames++;for(const key of Object.keys(r))sum[key]=(sum[key]??0)+r[key];return sum;},{frames:0}),drawStages:Object.fromEntries([...new Set(drawStages.flatMap(r=>Object.keys(r)))].map(key=>[key,{time:distribution(drawStages.map(r=>r[key]?.ms??0)),calls:drawStages.reduce((n,r)=>n+(r[key]?.calls??0),0)}])),glCalls:glCalls.reduce((sum,r)=>{for(const key of Object.keys(r))sum[key]=(sum[key]??0)+r[key];return sum;},{}),correctionBreakdown:{checkpointLookup:distribution(kernelTimings.filter(r=>r.phase==='lookup').map(r=>r.ms)),restoreTotal:distribution(kernelTimings.filter(r=>r.phase==='restore').map(r=>r.ms)),restoreCopy:distribution(snapshotTimings.filter(r=>r.phase==='restore').map(r=>r.copyMs)),restoreSelection:distribution(snapshotTimings.filter(r=>r.phase==='restore').map(r=>r.selectionMs)),restoreHost:distribution(snapshotTimings.filter(r=>r.phase==='restore').map(r=>r.hostMs)),replaySimulation:distribution(kernelTimings.filter(r=>r.phase==='replay').map(r=>r.ms)),checkpointForward:distribution(kernelTimings.filter(r=>r.phase==='checkpoint'&&!r.replay).map(r=>r.ms)),checkpointReplay:distribution(kernelTimings.filter(r=>r.phase==='checkpoint'&&r.replay).map(r=>r.ms))},audio:audio.snapshot(),audioJournalMatchesReference,kernel:kernel?.snapshot()??null,presentationBoundary:replica?'independent-replica':'conservative',replica:replicaMeasured,snapshotCosts:store.metrics(),cache:cache.snapshot(),gpuInfo:lastDraw?.gpuInfo,browser:navigator.userAgent,presentation:{width:1280,height:720,picture:[960,720],rect:picture.getBoundingClientRect().toJSON()},inputToPhoton:{measured:false,reason:'No physical input/display sensor; canvas capture is not photon timing.'},scope:'Diagnostic only; local retains native draw writes; detached modes isolate draw writes using the recorded presentation boundary; audio journal only.'};
  ...
  function tick(now){if(stopped)return;try{if(!clock.take(now,1)){requestAnimationFrame(tick);return;}const t=performance.now(),before=profileTimeline?kernel?.snapshot():null,kernelStart=kernelTimings.length,snapshotStart=snapshotTimings.length,drawStart=draw.length,copyStart=copy.length;let advanced=true,advanceMs=0;
   if(rollback){if(sent<kernel.frame){sent=kernel.frame;socket.send(JSON.stringify({type:'input',frame:sent,value:packets[sent][seat]}));}const a=performance.now();advanced=kernel.advance(packets[kernel.frame][seat]);advanceMs=performance.now()-a;advance.push(advanceMs);forward=kernel.frame;}
   else{measuredStep(packets[forward]);forward++;}
   let stateReadMs=0,presentationMs=0;if(advanced){let stateAt;if(profileTimeline)stateAt=performance.now();const current=boundary.readPlayers();if(profileTimeline)stateReadMs=performance.now()-stateAt;actualWorkload.hitlagFrames+=current.some(p=>p[14]>0);actualWorkload.damageFrames+=current.some(p=>p[13]>0);actualWorkload.attackFrames+=current.some(p=>p[0]>=44&&p[0]<=69);actualWorkload.stockChanges+=current.filter((p,i)=>p[18]!==lastForwardPlayers[i][18]).length;lastForwardPlayers=current;let presentationAt;if(profileTimeline)presentationAt=performance.now();present();if(profileTimeline)presentationMs=performance.now()-presentationAt;}const callbackMs=performance.now()-t;total.push(callbackMs);const after=kernel?.snapshot();if(profileTimeline)frameTimeline.push({frame:forward,advanced,rafMs:now-start,callbackStartMs:t-start,callbackMs,advanceMs,stateReadMs,presentationMs,drawMs:draw.length>drawStart?draw.at(-1):0,copyMs:copy.length>copyStart?copy.at(-1):0,submissionMs:advanced&&times.length?times.at(-1)-start:null,corrections:(after?.corrections??0)-(before?.corrections??0),replayedFrames:(after?.replayedFrames??0)-(before?.replayedFrames??0),kernelEvents:kernelTimings.slice(kernelStart),snapshotEvents:snapshotTimings.slice(snapshotStart)});globalThis.certificationProgress={forward,draws:draw.length,elapsedMs:performance.now()-start,kernel:after};
  ...
  function begin(){if(measuring)return;observer=observeCanvasFrames(picture,{enabled:params.get('observer')!=='0'});if(!detached)persistent=boundary.createPreview(cache);measuring=true;trackTimings=true;start=performance.now();clock=createNativeFrameClock(start,60,{align:true,toleranceMs:.25});requestAnimationFrame(tick);}
  let sent=-1;
  if(rollback){kernel=createRollbackSession({seat,store,step:measuredStep,onTiming:r=>{if(trackTimings)kernelTimings.push(r);},onReplay:v=>{replaying=v;if(v)replayStart=performance.now();else corrections.push(performance.now()-replayStart);}});socket=new WebSocket(new URL('/rollback-probe',location.href).href.replace(/^http/,'ws'));socket.onopen=()=>socket.send(JSON.stringify({type:'hello',seat,token:params.get('token')}));socket.onmessage=e=>{try{const m=JSON.parse(e.data);if(m.type==='start')begin();else if(m.type==='input')kernel.receive(m.frame,m.value);else if(m.type==='error')throw Error(m.message);}catch(e){stopped=true;fail(e);socket.close();}};socket.onerror=()=>fail(Error('Diagnostic relay failed'));}
  else begin();
