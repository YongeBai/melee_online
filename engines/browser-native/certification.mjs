...
if(mode==='calibration'){
 ...
 const runtime=await createSnapshotRuntime(create,bytes,{dirtyManifest:dirty?.manifest,onNativeMusic:r=>audio.request(r),onNativeAudioMode:()=>true}),module=runtime.module,cache=createPresentationCache({uniformBuffer:params.get('uniformbuffer')!=='0',exactState:params.get('exactstate')!=='0',compactShaderKey:params.get('shaderkey')!=='0',profileDraw:params.get('drawtiming')==='1',submissionOptimized:params.get('drawopt')!=='0',packedState:params.get('packedstate')!=='0',reuseImmediate:params.get('immediatereuse')==='0'?false:params.get('immediatereuse')==='1'?true:undefined,cacheImmediateViews:params.get('immediateviews')!=='0',cacheTextureView:params.get('textureview')!=='0'});
