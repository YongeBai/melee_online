...
if(mode==='calibration'){
 ...
 const runtime=await createSnapshotRuntime(create,bytes,{dirtyManifest:dirty?.manifest,onNativeMusic:r=>audio.request(r),onNativeAudioMode:()=>true}),module=runtime.module,cache=createPresentationCache({exactState:params.get('exactstate')!=='0',profileDraw:params.get('drawtiming')==='1',submissionOptimized:params.get('drawopt')!=='0',packedState:params.get('packedstate')!=='0',reuseImmediate:params.get('immediatereuse')==='0'?false:params.get('immediatereuse')==='1'?true:undefined});
 ...
 const boot=await runNativeConstructor({module,presentationCache:cache,canvas:picture,params:{tournament:1,hud:1,damagehud:1,intro:1,stagecallbacks:1,render:1,rendersteps:1,map:params.get('map')??'battlefield',character:params.get('character')??'Fc',opponent:params.get('opponent')??'Fx',gpuerrors:'deferred',...(params.get('map')==='fountain'?{fountaincosmetics:'off',fountainscenery:'off'}:{})},onMatchBoundary:async boundary=>{
  ...
  const frameOracleCache=frameOracleEnabled?createPresentationCache({exactState:false,submissionOptimized:false,packedState:false,reuseImmediate:false}):null,frameOracle={enabled:frameOracleEnabled,frames:0,comparedBytes:0,differentBytes:0,cameraMismatches:0};
  ...
  const corrections=[],sim=[],replaySim=[],advance=[],copy=[],capture=[],rebind=[],draw=[],dispose=[],restore=[],total=[],times=[],cameras=[],drawStages=[],glCalls=[];
  ...
  function drawFrame(preview){const t=performance.now();lastDraw=preview.draw();if(lastDraw.materialDraws.drawTiming)drawStages.push(lastDraw.materialDraws.drawTiming);if(lastDraw.materialDraws.exactGL)glCalls.push(lastDraw.materialDraws.exactGL);draw.push(performance.now()-t);observer.request();times.push(performance.now());cameras.push(camera(lastDraw));}
