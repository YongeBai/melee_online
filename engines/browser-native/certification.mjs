...
if(mode==='calibration'){
 ...
 const runtime=await createSnapshotRuntime(create,bytes,{onNativeMusic:r=>audio.request(r),onNativeAudioMode:()=>true}),module=runtime.module,cache=createPresentationCache();
 ...
 const boot=await runNativeConstructor({module,presentationCache:cache,canvas:picture,params:{tournament:1,hud:1,damagehud:1,intro:1,stagecallbacks:1,render:1,rendersteps:1,map:params.get('map')??'battlefield',character:params.get('character')??'Fc',opponent:params.get('opponent')??'Fx',gpuerrors:'deferred',...(params.get('map')==='fountain'?{fountaincosmetics:'off',fountainscenery:'off'}:{})},onMatchBoundary:async boundary=>{
  ...
  const corrections=[],sim=[],replaySim=[],advance=[],capture=[],rebind=[],draw=[],dispose=[],restore=[],total=[],times=[],cameras=[];
  ...
  function present(){if(replaying)throw Error('Presentation during replay');let before=null,preview=persistent,t=performance.now();
   ...
   try{t=performance.now();lastDraw=preview.draw();draw.push(performance.now()-t);observer.request();times.push(performance.now());cameras.push(camera(lastDraw));}
  ...
  async function finish(){stopped=true;finishTime=performance.now();const observation=await observer.stop();persistent?.validateGpu();persistent?.dispose();persistent=null;
   ...
   const pixels=()=>{const gl=picture.getContext('webgl2'),b=new Uint8Array(960*720*4);gl.readPixels(0,0,960,720,gl.RGBA,gl.UNSIGNED_BYTE,b);return b;};
   // Post-run corrected frame and independent fresh renderer, outside timing.
   try{oracle=boundary.createPreview(cache);cachedCamera=camera(oracle.draw());oracle.validateGpu();cachedPixels=pixels();}finally{oracle?.dispose();store.restore(final);}
   ...
   globalThis.certificationReport={diagnosticPassed:!failures.length,performanceCertified:false,failures,mode,seat,frames,elapsedMs:elapsed,simulationFps:1000*forward/elapsed,drawSubmissions:draw.length,submissionCadence:submission,observation,workload,actualWorkload,referenceHash,finalHash,stateMatches,measuredPlayersMatchReference:JSON.stringify(actualPlayers)===JSON.stringify(expectedPlayers),expectedPlayers,actualPlayers,pixelOracle:{differentBytes,comparedBytes:cachedPixels.length},camera:{sameAsFresh:sameCamera,corrected:cachedCamera,first:cameras[0],last:cameras.at(-1),fresh:oracleCamera,validatedEveryDraw:true},costs:{simulation:distribution(sim),correction:distribution(corrections),replaySimulation:distribution(replaySim),advanceIncludingCorrections:distribution(advance),presentationCapture:distribution(capture),rendererRebind:distribution(rebind),drawSubmission:distribution(draw),rendererDispose:distribution(dispose),presentationRestore:distribution(restore),forwardCallback:distribution(total)},kernel:kernel?.snapshot()??null,snapshotCosts:store.metrics(),cache:cache.snapshot(),gpuInfo:lastDraw?.gpuInfo,browser:navigator.userAgent,presentation:{width:1280,height:720,picture:[960,720],rect:picture.getBoundingClientRect().toJSON()},inputToPhoton:{measured:false,reason:'No physical input/display sensor; canvas capture is not photon timing.'},scope:'Diagnostic only; local retains normal native draw writes, isolated/rollback discard draw writes at detached snapshot boundary; audio journal only.'};
   document.querySelector('#result').textContent=JSON.stringify(certificationReport,null,2);kernel?.dispose();cache.dispose();store.release(final);store.release(reference);store.release(initial);store.dispose();socket?.close();
  }
