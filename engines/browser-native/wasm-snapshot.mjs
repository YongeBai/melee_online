...
export function instrumentSnapshotWasm(bytes){
 ...
 while(at<bytes.length){const id=bytes[at++],length=u(),start=at,end=start+length;if(end>bytes.length)throw Error('Truncated WASM section');
  ...
  if(id===5){if(u()!==1)throw Error('Snapshot requires one memory');const flags=u();if(flags!==1)throw Error('Snapshot requires unshared bounded memory');memory={initial:u(),maximum:u()};}
...
export function createWasmCheckpointStore({module,instance,audit,health={aborted:false},host={capture:()=>null,restore:()=>{}},maxBytes=1024*1024*1024}){
 ...
 function restore(key){const began=performance.now();guard();const r=row(key);if(module.HEAPU8.length!==r.bytes.length)throw Error('Memory growth across checkpoint is unsupported');host.restore(structuredClone(r.host));module.HEAPU8.set(r.bytes);globals.forEach((g,i)=>g.value=r.globals[i]);const ms=performance.now()-began;metrics.restores++;metrics.restoredBytes+=r.bytes.length;metrics.restoreCpuMs+=ms;metrics.maxRestoreCpuMs=Math.max(metrics.maxRestoreCpuMs,ms);}
