...
export function createNativeProductUI({module,input,menu,canvas}){
 ...
 panel.innerHTML='<canvas id="roomStatus" role="status"></canvas><div class="room-code"><img src="./assets/room-code.png" alt="Your code"><button id="copyRoom" aria-label="Copy room code"><canvas id="ownCode"></canvas></button></div><form id="joinRoom"><label><img src="./assets/room-join.png" alt="Join room"><input id="joinCode" aria-label="Room code" maxlength="6" autocomplete="off" spellcheck="false"></label><button type="submit" aria-label="Join room">▶</button></form><button id="readyRoom"><img src="./assets/room-ready.png" alt="Ready"></button><button id="cpuRoom"><canvas></canvas></button><button id="kickRoom"><canvas></canvas></button><button id="leaveRoom"><canvas></canvas></button>';
 ...
 const peer=document.createElement('span');peer.id='peerKeyboard';peer.textContent='CPU 9';viewport.append(peer);
 const dialog=document.createElement('dialog');dialog.id='controls';dialog.setAttribute('aria-label','Keyboard controls');
 dialog.innerHTML='<div class="controls-surface"><div id="keyboardModel" role="img" aria-label="3D keyboard controls"></div><label class="tap-setting"><img src="./assets/tap-jump.png" alt="Tap jump"><input id="tapJump" type="checkbox" checked><img id="tapState" src="./assets/on.png" alt="On"></label><button id="closeControls" aria-label="Back"><img src="./assets/back.png" alt="Back"></button></div>';document.body.append(dialog);
 ...
 const show=()=>{if(scene!=='characters'||dialog.open)return;input.suspend(true);dialog.showModal();model??=createKeyboardModel($('keyboardModel'));model.setVisible(true);modalB=!!navigator.getGamepads?.()[0]?.buttons?.[2]?.pressed;modalRaf=requestAnimationFrame(pollModal);};
 const close=()=>{cancelAnimationFrame(modalRaf);dialog.close();model?.setVisible(false);input.suspend(false);};keyboard.onclick=show;$('closeControls').onclick=close;dialog.addEventListener('cancel',e=>{e.preventDefault();close();});
 try{$('tapJump').checked=localStorage.getItem('native-melee-tap-jump')!=='0';}catch{}module._portTapJumpSet(input.keyboardPort,+$('tapJump').checked);$('tapState').src='./assets/'+($('tapJump').checked?'on':'off')+'.png';
 $('tapJump').onchange=()=>{try{localStorage.setItem('native-melee-tap-jump',$('tapJump').checked?'1':'0');}catch{}module._portTapJumpSet(input.keyboardPort,+$('tapJump').checked);network?.setTapJump(+$('tapJump').checked);$('tapState').src='./assets/'+($('tapJump').checked?'on':'off')+'.png';};
 ...
 $('joinCode').addEventListener('focus',()=>input.suspend(true));$('joinCode').addEventListener('blur',()=>input.suspend(false));$('joinCode').oninput=e=>{e.target.value=e.target.value.toUpperCase().replace(/[^A-Z2-9]/g,'');};
 ...
 function refresh(){const state=network?.state;message(cpu?'CPU Level 9':state?.hasGuest?state.connected.every(Boolean)?state.ready.every(Boolean)?'Starting':state.ready[state.seat]?'Waiting for Ready':'Player connected':'Player disconnected':'Waiting for player');void lettering($('cpuRoom').querySelector('canvas'),cpu?'Remove CPU':'Play CPU Lv 9');void lettering($('kickRoom').querySelector('canvas'),'Kick');void lettering($('leaveRoom').querySelector('canvas'),'Leave');peer.textContent=cpu?'CPU 9':network?.seat?'P1':'P2';$('joinRoom').hidden=!!state?.hasGuest;$('readyRoom').hidden=!cpu&&!network?.connected;$('cpuRoom').hidden=!!network?.offline||!!state?.seat||!!state?.hasGuest;panel.querySelector('.room-code').hidden=!!network?.offline;$('kickRoom').hidden=!!state?.seat||!state?.hasGuest;$('leaveRoom').hidden=!state?.seat;$('readyRoom').disabled=!!state?.ready[state.seat];}
 refresh();void lettering($('ownCode'),'');
 return {setNetwork(value){network=value;network.setTapJump(+$('tapJump').checked);if(cpu!==value.cpu){cpu=value.cpu;if(scene==='characters')module._portMenuSetCPU(+cpu);}void lettering($('ownCode'),value.code);refresh();},update(s){scene=s.scene;position();panel.hidden=keyboard.hidden=peer.hidden=scene!=='characters';if(dialog.open&&scene!=='characters')close();},message,close,connectionError(error){close();input.suspend(true);notice.querySelector('p').textContent=error.message;notice.hidden=false;}};
}
