...
export function createNativeProductUI({module,input,menu,canvas}){
 ...
 const peer=document.createElement('span');peer.id='peerKeyboard';peer.innerHTML=keyboard.innerHTML;peer.classList.add('cpu');peer.setAttribute('aria-label','Level 9 CPU');viewport.append(peer);
 ...
 function refresh(){const state=network?.state;message(cpu?'CPU Level 9':state?.hasGuest?state.connected.every(Boolean)?state.ready.every(Boolean)?'Starting':state.ready[state.seat]?'Waiting for Ready':'Player connected':'Player disconnected':'Waiting for player');void lettering($('cpuRoom').querySelector('canvas'),cpu?'Remove CPU':'Play CPU Lv 9');void lettering($('kickRoom').querySelector('canvas'),'Kick');void lettering($('leaveRoom').querySelector('canvas'),'Leave');peer.classList.toggle('cpu',cpu);peer.setAttribute('aria-label',cpu?'Level 9 CPU':network?.seat?'Player 1 keyboard':'Player 2 keyboard');$('readyRoom').classList.toggle('selected',!!state?.ready[state.seat]);$('joinRoom').hidden=!!state?.hasGuest;$('readyRoom').hidden=!cpu&&!network?.connected;$('cpuRoom').hidden=!!network?.offline||!!state?.seat||!!state?.hasGuest;panel.querySelector('.room-code').hidden=!!network?.offline;$('kickRoom').hidden=!!state?.seat||!state?.hasGuest;$('leaveRoom').hidden=!state?.seat;$('readyRoom').disabled=!!state?.ready[state.seat];}
 refresh();void lettering($('ownCode'),'');
 return {setNetwork(value){network=value;document.body.classList.add('online-room');document.body.dataset.seat=value.seat;keyboard.setAttribute('aria-label','Player '+(value.seat+1)+' keyboard controls');network.setTapJump(+$('tapJump').checked);if(cpu!==value.cpu){cpu=value.cpu;if(scene==='characters')module._portMenuSetCPU(+cpu);}void lettering($('ownCode'),value.code);refresh();},update(s){scene=s.scene;position();panel.hidden=keyboard.hidden=peer.hidden=scene!=='characters';if(dialog.open&&scene!=='characters')close();},message,close,connectionError(error){close();input.suspend(true);notice.querySelector('p').textContent=error.message;notice.hidden=false;}};
}
