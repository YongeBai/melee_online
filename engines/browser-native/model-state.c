...
void portModelCaptureReset(void){require(1);memset(&state,0,offsetof(ModelState,position));}
...
void GXLoadNrmMtxImm(f32 mtx[3][4],u32 id){unsigned i=slot(id);require(mtx!=NULL);memset(state.normal[i],0,sizeof(Mtx));for(unsigned r=0;r<3;r++)for(unsigned c=0;c<3;c++)state.normal[i][r][c]=mtx[r][c];state.normal_mask|=1u<<i;}
