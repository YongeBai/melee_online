...
export function createRenderReplica(source,target,{sourceHost,targetHost,copyMode="full",auditDirty=false,verifyImmutableTable=false}={}){
 ...
 let busy=false,closed=false;const stats={frames:0,copiedBytes:0,comparedBytes:0,copyCpuMs:0,constructCpuMs:0,drawCpuMs:0,disposeCpuMs:0,guardCpuMs:0,tableEntriesChecked:0,fixedTableContents,scanTable,maxCopyCpuMs:0,maxConstructCpuMs:0,maxDrawCpuMs:0,maxDisposeCpuMs:0,replicaBytes:memoryBytes,copyMode,dirtyPages:0,fullCopies:0,coverageAudits:0,dirtySamples:[],timingSamples:[]};
 ...
 return {
  present(construct,draw){
   ...
   const sourceGlobals=states[0].globals.map(g=>g.value),t=performance.now(),timing={frame:stats.frames};
   try{
    ...
    const ms=performance.now()-t;timing.copyMs=ms;stats.copyCpuMs+=ms;stats.maxCopyCpuMs=Math.max(stats.maxCopyCpuMs,ms);
    const constructStart=performance.now();renderer=construct(target.module);timing.constructMs=performance.now()-constructStart;stats.constructCpuMs+=timing.constructMs;stats.maxConstructCpuMs=Math.max(stats.maxConstructCpuMs,timing.constructMs);
    const drawStart=performance.now(),result=draw(renderer);timing.drawMs=performance.now()-drawStart;stats.drawCpuMs+=timing.drawMs;stats.maxDrawCpuMs=Math.max(stats.maxDrawCpuMs,timing.drawMs);
    if(result?.then)throw Error('Replica draw must be synchronous');stats.frames++;return result;
   }finally{
    try{const disposeStart=performance.now();renderer?.dispose();timing.disposeMs=performance.now()-disposeStart;stats.disposeCpuMs+=timing.disposeMs;stats.maxDisposeCpuMs=Math.max(stats.maxDisposeCpuMs,timing.disposeMs);if(stats.timingSamples.length<16)stats.timingSamples.push(timing);detached(target);detached(source);if(states[0].globals.some((g,i)=>g.value!==sourceGlobals[i]))throw Error('Rendering changed gameplay globals');}finally{busy=false;}
   }
  },
