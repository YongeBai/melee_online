...
export async function createNativeProductRollback({source,wasmBytes,dirtyManifest=null,audio,network,step,createPreview,presentationCache=null}){
 ...
 const store=createPagedWasmCheckpointStore({...source,host:audio,sparse:dirty,maxBytes:1024**3});let session,driver,closed=false,lastCoverage=null,lastDraw=null;const warmup={frames:30,cpuMs:0};
 ...
 return {driver,preview,snapshot:()=>({warmup:{...warmup},session:session.snapshot(),replica:replica.metrics(),snapshots:store.metrics(),presentationCache:cache.snapshot(),lastDraw:lastDraw&&{resolution:lastDraw.resolution,eye:lastDraw.eye,interest:lastDraw.interest,fov:lastDraw.fov,aspect:lastDraw.aspect}})};
}
