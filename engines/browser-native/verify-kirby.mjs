...
export function verifyKirbyCopy(module,objects,report,{step,onStep=()=>{},mode='swallow'}){
  ...
  require(read()[0][11]===4&&read()[1][11]===0,'Kirby versus Mario');require(['swallow','acquire','spit'].includes(mode),'mode');
  Object.assign(report,{completed:false,frames:0,states:[[],[]],trace:[],itemKinds:[],retailParityVerified:false});
  ...
  function tick(a=[0,0,0],b=[0,0,0]){
    ...
    const n=module._portItemsList(buffer,128);require(n<=128,'item capacity');
    const items=Array.from(new Uint32Array(module.HEAPU8.buffer,buffer,n),o=>Array.from({length:9},(_,i)=>module._portItemRead(o,i)));
    for(const item of items){require(item.every(Number.isFinite)&&(!(item[0]===130)||item[5]===objects[0]),'finite items and Kirby projectile ownership');if(!report.itemKinds.includes(item[0]))report.itemKinds.push(item[0]);}
  ...
  try{
    ...
    require(report.hat[0]===0&&report.hat[1]&&report.hat[2],'original Mario hat acquired');require(read().every(s=>s[0]===14),'both fighters recover');
    report.firstHat=report.hat.slice();
    phase='copied fireball';tick([0x200,0,0]);neutral(240);
    require(report.states[0].includes(399)&&report.itemKinds.includes(130),'original copied fireball state and item');
    require(read()[0][0]===14&&report.hat[0]===0,'copy retained after attack');require(!module._portItemsList(buffer,128),'fireball retirement');
    if(mode==='acquire'){report.completed=true;return;}
    phase='air copied fireball';for(let i=0;i<10;i++)tick([0x400,0,0]);neutral(8);tick([0x200,0,0]);neutral(300);require(report.states[0].includes(400)&&read()[0][0]===14,'air copy and recovery');
    ...
    phase='reacquire swallow';tick([0,0,-1]);neutral(140);require(report.hat[0]===0&&report.hat[1],'hat recreated');report.secondHat=report.hat.slice();
    phase='reacquired fireball';tick([0x200,0,0]);neutral(240);require(report.trace.some(t=>t.phase===phase&&t.items.some(i=>i[0]===130)),'reacquired projectile');require(read()[0][0]===14&&report.hat[0]===0&&!module._portItemsList(buffer,128),'reacquired copy retained and item retired');
    report.completed=true;
  }finally{module._free(buffer);}
}
