...
export function verifyKirbyCopy(module,objects,report,{step,onStep=()=>{},mode='swallow',donor='primary'}){
  ...
  function tick(a=[0,0,0],b=[0,0,0]){
    ...
    const egg=profile.eggLay?[module._portFighterAccessory(objects[1],0),module._portFighterAccessory(objects[1],1)]:null;
    report.trace.push({phase,state:s,hat,items,attachments,charge,body,sword,flame,hammer,climbers,egg});report.final=s;report.hat=hat;onStep(report);return s;
  }
  ...
  try{
    ...
    if(profile.eggLay){
      ...
      if(mode!=='contact')for(const air of [false,true]){
        phase=(air?'air':'ground')+' copied tongue miss';approach(65);
        if(air){for(let i=0;i<10;i++)tick([0x400,0,0]);neutral(8);}
        const start=report.trace.length;tick([0x200,0,0]);neutral(200);
        const rows=report.trace.slice(start);require(rows.some(t=>t.state[0][0]===(air?profile.air:profile.ground))&&!rows.some(t=>[331,332].includes(t.state[1][0]))&&read().every(s=>s[0]===14),'original tongue miss and recovery');
      }
      phase='copied egg capture approach';approach(profile.contactDistance);report.contactBefore=read();
      phase='ground copied egg capture';const first=report.trace.length;tick([0x200,0,0]);neutral(600);
      const rows=report.trace.slice(first);report.eggAccessoryFrames=rows.filter(t=>t.egg[1]===9).length;
      report.contactAfter=read();require(rows.some(t=>t.state[1][0]===331)&&rows.some(t=>t.state[1][0]===332)&&report.eggAccessoryFrames>0&&report.contactAfter[1][13]>report.contactBefore[1][13],'copied tongue capture, egg shell and damage');
      require(read().every(s=>s[0]===14)&&module._portFighterAccessory(objects[1],1)!==9&&report.hat[0]===copyKind,'egg escape, accessory cleanup and copy retention');
      if(mode==='contact'){report.completed=true;return;}
      phase='air copied egg approach';approach(profile.contactDistance);
      tick([0x400,0,0]);neutral(4);require(read()[0][3]===1,'airborne copied egg setup');
      phase='air copied egg capture';const airStart=report.trace.length,airDamage=read()[1][13];tick([0x200,0,0],[0x400,0,0]);neutral(600);
      const airRows=report.trace.slice(airStart);report.airEggAccessoryFrames=airRows.filter(t=>t.egg[1]===9).length;
      require(airRows.some(t=>t.state[0][0]===419&&t.state[0][3]===1)&&airRows.some(t=>t.state[1][0]===332)&&report.airEggAccessoryFrames>0&&read()[1][13]>airDamage,'airborne tongue capture and egg release');
      require(read().every(s=>s[0]===14)&&module._portFighterAccessory(objects[1],1)!==9&&report.hat[0]===copyKind,'air egg escape and cleanup');
    }else if(profile.rollout){
      if(mode==='acquire'){report.completed=true;return;}
