...
void portTournamentInitializeStage(unsigned left,unsigned right,unsigned stage)
{
    ...
    if(initialized||characters[0]<0||characters[1]<0||(stage!=St_Kind_Battle&&stage!=St_Kind_Last&&stage!=St_Kind_OldPupupu&&stage!=St_Kind_Izumi))abort();StartMeleeData data={0};
