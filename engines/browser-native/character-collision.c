/* Original hurtbox initialization/world-position routines in a limited Fighter
 * fixture. Combat, parts/material setup and dynamic-bone simulation are separate
...
typedef struct { Fighter fighter; ftData data; ftDynamics dynamics; FighterBone parts[140]; } CollisionFixture;
static CollisionFixture* context(HSD_GObj* object){if(!object||!object->user_data)abort();return object->user_data;}
int portCollisionAttach(HSD_GObj* object,CollisionData* data,unsigned count,HSD_JObj** parts)
{
    if(!object||object->user_data||!data||count>140||data->hurt.count<0||data->hurt.count>15||data->count<0||data->count>11)return -1;
    CollisionFixture* c=calloc(1,sizeof(*c));if(!c)return -2;
    c->fighter.gobj=object;c->fighter.ft_data=&c->data;c->fighter.parts=c->parts;
    c->data.x30=&data->hurt;c->data.x2C=&c->dynamics;c->dynamics.x4=data->count;c->dynamics.x8=data->dynamics;
    for(unsigned i=0;i<count;i++)c->parts[i].joint=parts[i];
    ...
    GObj_InitUserData(object,4,free,c);ftColl_8007B320(object);return 0;
}
