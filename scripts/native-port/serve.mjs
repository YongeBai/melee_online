...
for(const manifestName of ['menu-fixtures.json','effect-fixtures.json','shared-fixtures.json','stage-fixtures.json','fighter-fixtures.json','animation-fixtures.json','model-fixtures.json']) {
...
if(fs.existsSync(manifest))for(const name of JSON.parse(fs.readFileSync(manifest))) {
  if(name!=='MnSlMap.usd'&&name!=='MnSlChr.usd'&&name!=='SdSlChr.usd'&&name!=='GmPause.usd'&&name!=='PdPm.dat'&&name!=='IfAll.usd'&&name!=='ItCo.usd'&&!/^(Gr|Pl|Ef)[A-Za-z0-9]+\.(dat|usd)$/.test(name))throw Error('Invalid fixture filename');
