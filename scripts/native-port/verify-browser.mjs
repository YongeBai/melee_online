...
try {
  ...
  if(stageMap){
    ...
    if(fighterInit&&(!verification.itemModels?.passed||verification.itemModels.rows.length!==27||verification.itemModels.rows.flatMap(r=>r.articles).length!==80||verification.itemModels.instances!==160||verification.itemModels.updates!==640||verification.itemModels.packedFlagChecks!==65536))throw Error('Incomplete original item model and hurtbox coverage');
    ...
    if(fighterInit&&(!verification.commandFields?.passed||verification.commandFields.fields!==277||verification.dynamics?.rows.some(r=>!r.animation?.passed||r.animation.clips.length!==3||r.animation.instances!==2||r.animation.frames!==192)))throw Error('Incomplete original fighter animation integration');
