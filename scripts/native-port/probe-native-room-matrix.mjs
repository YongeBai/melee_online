...
const output=path.join(root,'dist/native-port/experiment-native-room-matrix');fs.mkdirSync(output,{recursive:true});const rows=[];
for(const [index,entry] of cases.entries()){
 ...
 const row={index,pair:entry.pair,stage:entry.stage,stageName:first.stage.name,holdASeat:entry.holdASeat,selection:first.selection.players.slice(0,2),initialKinds:first.live.initial.map(v=>v[11]),fighterForms:first.fighterForms.map(v=>({slot:v.slot,index:v.index,code:v.code})),frames:first.live.frames,draws:first.live.draws,confirmed:report.final.map(v=>v.report.rollback.metrics.session.confirmed),matchingFighterState:true};rows.push(row);console.log(JSON.stringify(row));
}
