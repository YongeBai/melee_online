...
export function readMotionSpec(upstream) {
  ...
  const demo=/ftData_UnkIntPairs\[Ft_Kind_Max\] = \{([\s\S]*?)\n\};/.exec(data)?.[1];
  const load=/ftData_OnLoad\[Ft_Kind_Max\] = \{([\s\S]*?)\n\};/.exec(data)?.[1];
  const lengths=/static u8 ftAction_803C0870\[[^\n]+ = \{([\s\S]*?)\n\};/.exec(action)?.[1];
  if(!table||!demo||!load||!lengths)throw Error('Original motion metadata declarations changed');
  const counts=[...table.matchAll(/\{\s*0,\s*(\d+)\s*\}/g)].map(m=>Number(m[1]));
  const demoCounts=[...demo.matchAll(/\{\s*0,\s*(\d+)\s*\}/g)].map(m=>Number(m[1]));
  ...
  if(counts.length!==33||demoCounts.length!==33||codes.length!==33||actionWords.length!==49||actionWords.some(n=>!Number.isInteger(n)||n<1||n>7))
    throw Error('Original motion metadata dimensions changed');
  return {codes,counts,demoCounts,commandWords:[1,1,1,1,1,2,1,2,1,1,...actionWords],
