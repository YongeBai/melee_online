...
test('frame staging belongs to the exclusive lease and is discarded with the cache',()=>{
 ...
 const a=cache.acquire(gl),first=a.modelSnapshot(0,()=>{creates++;return {rows:new Float32Array(120)};});assert.equal(a.packedState,true);assert.equal(a.reuseImmediate,true);assert.equal(a.compactShaderKey,true);a.release();
 ...
 const control=createPresentationCache({packedState:false,compactShaderKey:false}),c=control.acquire(gl);assert.equal(c.packedState,false);assert.equal(c.compactShaderKey,false);assert.equal(control.snapshot().compactShaderKey,false);c.release();control.dispose();
});
