...
test('aliased Emscripten import namespaces mark WASI outputs without invalidating draw callbacks',{skip:!fs.existsSync(assembler)},async()=>{
 const {createSnapshotRuntime}=await import('../../engines/browser-native/wasm-snapshot.mjs'),{createDirtyRangeTracker,subscribeDirtyPages}=await import('../../engines/browser-native/dirty-runtime.mjs'),{createHash}=await import('node:crypto');
 ...
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'dirty-imports-'));try{
  ...
  const runtime=await createSnapshotRuntime(create,bytes,{dirtyManifest:{instrumentedSha256:createHash('sha256').update(bytes).digest('hex'),originalSha256:'fixture'}}),subscription=subscribeDirtyPages(runtime,'test'),tracker=createDirtyRangeTracker(runtime),range=runtime.module.HEAPU8.subarray(8000,10000);subscription.clear();runtime.dirty.fill(0);imports.env.portDispatchObject(0,0,0,0,0);assert(runtime.dirty.every(x=>x===0));
  const before=tracker.stamp([range]);imports.wasi_snapshot_preview1.fd_write(0,0,0,9000);assert.equal(runtime.dirty[2],1);assert.equal(runtime.dirty.reduce((a,b)=>a+b,0),1);assert.equal(new DataView(runtime.module.HEAPU8.buffer).getUint32(9000,true),42);const after=tracker.stamp([range]);assert.notDeepEqual(after,before);assert.equal(subscription.read()[2],1);subscription.clear();assert.deepEqual(tracker.stamp([range]),after);assert.throws(()=>tracker.stamp([new Uint8Array(1)]),/ownership/);subscription.dispose();const replacement=subscribeDirtyPages(runtime,'replacement');replacement.clear();runtime.dirty[2]=1;tracker.stamp([range]);assert.equal(replacement.read()[2],1);replacement.dispose();tracker.dispose();assert.throws(()=>tracker.stamp([range]),/unavailable/);
