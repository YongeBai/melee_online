...
const flags=['-O2','-fno-fast-math','-ffp-contract=off','-fno-strict-aliasing',
  ...
  '-I'+path.join(native,'include'),'-I'+path.join(native,'portable/src'),
  '-I'+path.join(native,'portable/libs/dolphin/include'),'-Isrc','-Ilibs/dolphin/include'];
...
for(const entry of ['HSD_JObjLoadJoint','Fighter_FirstInitialize_80067A84','Fighter_UnkInitLoad_80068914','Fighter_Create']) {
  ...
  results.push({entry,linked:result.status===0,missingCount:missing.length,missing,
    abiMismatches:diagnostics.split('\n').filter(line=>line.includes('function signature mismatch')),
...
const report={source,scope:'Reachable game/HSD objects with implemented native arithmetic, SDK projection, heap and panic boundaries',
  portableSource:audit.portableSource,
