...
try {
  ...
  await command('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+'/constructor.html'+(combat?'?step=1&stage=1&combat=1'+(control?'&control=1':'')+(camera?'&camera=1':'')+(render?'&render=1':'')+(callbacks?'&callbacks=1':'&callbacks=0')+(tournament?'&tournament=1':'')+(hud?'&hud=1':'')+(damageHud?'&damagehud=1':'')+(intro?'&intro=1':'')+(stageCallbacks?'&stagecallbacks=1':'')+(process.argv.includes('--timeout')?'&timeout=1':'')+(live?'&live=1&liveframes='+liveFrames:'')+(workload?'&workload=1':'')+(workloadSteps?'&workloadsteps='+liveFrames:'')+(renderSteps?'&rendersteps=1':''):input?'?step=1&stage=1&input=1'+(render?'&render=1':'')+(renderSteps?'&rendersteps=1':''):stage?'?step=1&stage=1':step?'?step=1':'?probe=1')+(input&&combat?'&input=1':'')+(purinContact?'&purincontact='+purinContact:'')+(purinMoves?'&purinmoves=1':'')+(marioMoves?'&mariomoves=1':'')+(pikachuMoves?'&pikachumoves=1':'')+(samusMoves?'&samusmoves=1':'')+(samusContact?'&samuscontact='+samusContact:'')+(linkContact?'&linkcontact='+linkContact:'')+(linkMoves?'&linkmoves=1':'')+(yoshiMoves?'&yoshimoves=1':'')+(gamewatchContact?'&gamewatchcontact='+gamewatchContact:'')+(gamewatchMoves?'&gamewatchmoves=1':'')+(mewtwoMoves?'&mewtwomoves=1':'')+(mewtwoContact?'&mewtwocontact='+mewtwoContact:'')+(yoshiContact?'&yoshicontact='+yoshiContact:'')+(koopaMoves?'&koopamoves=1':'')+(koopaContact?'&koopacontact='+koopaContact:'')+(projectiles?'&projectiles=1':'')+(projectileCombat?'&projectilecombat=1':'')+(projectileControl?'&projectilecontrol=1':'')+(projectileShield?'&projectileshield=1':'')+(projectileReflect?'&projectilereflect=1':'')+(stageOnly?'&stageonly=1':'')+'&stageframes='+stageFrames+(deferredGpuErrors?'&gpuerrors=deferred':'')+(recordShaders?'&recordshaders=1':'')+(prewarmShaders?'&prewarmshaders=1':'')+(stadiumFireworksOff?'&stadiumfireworks=off':'')+(fountainSceneryOff?'&fountainscenery=off':'')+(fountainCosmeticsOff?'&fountaincosmetics=off':'')+'&map='+stageKey+'&character='+character+(process.argv.includes('--verify-vertices')?'&verifyvertices=1':'')});
  if(cpuProfile){
    let first;
    for(let i=0;i<120;i++){
  ...
  const probeSeconds=renderSteps&&process.argv.includes('--verify-vertices')?(linkMoves?(stageCallbacks?1800:900):stageCallbacks?600:300):live||renderSteps||stageOnly?90:20;
  ...
  for(const name of ['gwchef','gwjudge','gwrescue','gwturtle','shadowball','disable','teleport','capturedegg','eggshield','eggthrow','eggroll','yoshistars','linkhit','arrow','hookshot','boomerang','flame','claw','throw','charge','missile','grapple','jolt','thunder'])if(probe.preview?.[name]&&!probe.error){
    const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-"+name+"').src",returnByValue:true});
    if(!shot.result.value?.startsWith('data:image/png;base64,'))throw Error('Missing move screenshot');
    fs.writeFileSync(path.join(output,character+'-native-'+name+'.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
  ...
  if(probe.preview?.cape&&!probe.error){
    ...
    fs.writeFileSync(path.join(output,character+'-native-cape.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
  ...
  if(recordShaders&&!probe.error){
    ...
    catalog.recording={stage:stageKey,character,stadiumFireworksOff,fountainCosmeticsOff,fountainSceneryOff,frames:probe.live?.frames??probe.renderedSteps??probe.input?.frames??probe.combat?.frames??0,stageCallbackFrames:probe.stage?.callbackFrames??0,workload,stageOnly,inheritedPrograms:probe.shaderPreparation?.compiled??0};
