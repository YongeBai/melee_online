...
if(projectiles&&(!input||!['Fx','Fc','Mr','Lg','Dr'].includes(character))||[projectileControl,projectileShield,projectileReflect].filter(Boolean).length>1)throw Error('Projectile probes require a supported character with --input and one contact mode');
...
try {
  ...
  const report={stage:stageKey,character,marioMoves,purinMoves,purinContact,stadiumFireworksOff,fountainCosmeticsOff,fountainSceneryOff,recordShaders,prewarmShaders,deferredGpuErrors,driverShaderCacheDisabled:process.env.MESA_SHADER_CACHE_DISABLE==='true',rendererSources,cpuProfileInstrumented:cpuProfile,gpuRequested:hardware?'hardware':'software',browser:execFileSync(chrome,['--version'],{encoding:'utf8'}).trim(),build:JSON.parse(fs.readFileSync(path.join(output,'fighter-init-build.json'))),probe};
  if(recordShaders&&!probe.error){
    ...
    catalog.recording={stage:stageKey,character,stadiumFireworksOff,fountainCosmeticsOff,fountainSceneryOff,frames:probe.live?.frames??probe.input?.frames??probe.combat?.frames??0,stageCallbackFrames:probe.stage?.callbackFrames??0,workload,stageOnly,inheritedPrograms:probe.shaderPreparation?.compiled??0};
