...
const projectileControl=process.argv.includes('--projectile-control'),projectileShield=process.argv.includes('--projectile-shield'),projectileCombat=projectileControl||projectileShield||process.argv.includes('--projectile-combat'),projectiles=projectileCombat||process.argv.includes('--projectiles');
if(projectiles&&(!input||character!=='Fc')||projectileControl&&projectileShield)throw Error('Projectile probes require --character=Fc --input and one contact mode');
...
try {
  ...
  if(probe.preview?.projectile&&!probe.error){
    ...
    fs.writeFileSync(path.join(output,character+'-native-match-projectile.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
