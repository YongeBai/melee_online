...
try {
  ...
  await command('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+'/constructor.html'+(step?'?step=1':'')});
  ...
  fs.writeFileSync(path.join(output,step?'constructor-step-probe.json':'constructor-probe.json'),JSON.stringify(report,null,2)+'\n');fs.writeFileSync(path.join(output,'constructor-chrome.log'),stderr);
  console.log(JSON.stringify(probe,null,2));if(!probe.constructorVerified||probe.error||(step&&probe.schedulerSteps!==120))process.exitCode=2;
} finally {
