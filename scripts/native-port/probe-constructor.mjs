...
const control=process.argv.includes('--combat-control'),combat=control||process.argv.includes('--combat'),input=process.argv.includes('--input'),stage=combat||input||process.argv.includes('--stage'),step=stage||process.argv.includes('--step');
...
try {
  ...
  await command('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+'/constructor.html'+(combat?'?step=1&stage=1&combat=1'+(control?'&control=1':''):input?'?step=1&stage=1&input=1':stage?'?step=1&stage=1':step?'?step=1':'')});
  ...
  fs.writeFileSync(path.join(output,control?'constructor-combat-control-probe.json':combat?'constructor-combat-probe.json':input?'constructor-input-probe.json':stage?'constructor-stage-probe.json':step?'constructor-step-probe.json':'constructor-probe.json'),JSON.stringify(report,null,2)+'\n');fs.writeFileSync(path.join(output,'constructor-chrome.log'),stderr);
