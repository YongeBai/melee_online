...
const cacheModels=!process.argv.includes('--no-cache-models');
if(process.argv.includes('--cache-models')&&!cacheModels)throw Error('Choose one model cache mode');
...
const costume=Number(process.argv.find(x=>x.startsWith('--costume='))?.slice(10)??0),opponentCostume=Number(process.argv.find(x=>x.startsWith('--opponent-costume='))?.slice(19)??0);
if([costume,opponentCostume].some(x=>!Number.isInteger(x)||x<0||x>5))throw Error('Invalid costume index');
const matchup=(opponent===character?character:character+'-vs-'+opponent)+(costume||opponentCostume?'-colors-'+costume+'-'+opponentCostume:'');
const pauseInput=process.argv.includes('--pause-input');
if(pauseInput&&(!process.argv.includes('--input')||!process.argv.includes('--stage-callbacks')))throw Error('Pause probe requires input and stage callbacks');
...
if(kirbyCopy&&(character!=='Kb'||!['Mr','Lg','Dr','Ca','Gn','Ns','Pe','Fx','Pk','Pc','Lk','Cl','Ss','Fc','Dk','Ms','Fe','Zd','Sk','Kp','Pp','Mt','Pr','Gw','Ys'].includes(opponent)||!process.argv.includes('--input')||!['swallow','acquire','spit','contact','reflect','reflect-control'].includes(kirbyCopy)||kirbyCopy.startsWith('reflect')&&opponent!=='Zd'))throw Error('Kirby copy requires Kb versus Mr/Lg/Dr/Ca/Gn/Ns/Pe/Fx/Pk/Pc/Lk/Cl/Ss/Fc/Dk/Ms/Fe/Zd/Sk/Kp/Pp/Mt/Pr/Gw/Ys and input');
if(kirbyCopy&&opponent==="Gw"&&process.argv.includes("--render-steps")&&!process.argv.includes("--stage-callbacks"))throw Error("Game & Watch copy rendering requires --stage-callbacks for original outline passes");
...
const climbersMove=process.argv.find(x=>x.startsWith('--climbers-move='))?.slice(16);
const climbersMoves=!!climbersMove||process.argv.includes('--climbers-moves');
if(climbersMoves&&(character!=='Pp'||!process.argv.includes('--input')||climbersMove&&climbersMove!=='belay'))throw Error('Climber moves require Pp, input and optional belay');
...
const livePause=process.argv.includes('--live-pause');
...
const illusionAttachments=process.argv.includes('--illusion-attachments');
if(illusionAttachments&&(!['Fx','Fc'].includes(character)||!process.argv.includes('--input')||!process.argv.includes('--render-steps')||!process.argv.includes('--stage-callbacks')))throw Error('Illusion attachment probe requires Fx/Fc, input, render steps and stage callbacks');
...
if(livePause&&(!live||workload||!stageCallbacks||liveFrames<900))throw Error('Live pause requires live, original stage callbacks, no workload and at least 900 frames');
...
let liveTransformProof,liveControllerProof,livePauseProof;
...
try {
  ...
  if(live&&!workload){
    ...
    if(livePause){
      ...
      await keyEvent('Enter','Enter','keyDown');await waitFor('globalThis.nativeLive?.snapshot().match?.pause[0]===1');await keyEvent('Enter','Enter','keyUp');
      const read=async()=>(await command('Runtime.evaluate',{expression:'globalThis.nativeLive.snapshot()',returnByValue:true})).result.value;
      const paused=await read();await keyEvent('ArrowRight','ArrowRight','keyDown');
      await waitFor('globalThis.nativeLive?.snapshot().frames>='+String(paused.frames+30));await keyEvent('ArrowRight','ArrowRight','keyUp');
      const held=await read();if(JSON.stringify(paused.final)!==JSON.stringify(held.final)||JSON.stringify(paused.match.clock)!==JSON.stringify(held.match.clock)||held.match.pause[0]!==1)throw Error('Live pause did not freeze native match');
      await keyEvent('Escape','Escape','keyDown');await waitFor('globalThis.nativeLive?.snapshot().match?.pause[0]===0');await keyEvent('Escape','Escape','keyUp');
      await waitFor('globalThis.nativeLive?.snapshot().match?.clock[0]>'+String(paused.match.clock[0]+10));
      const resumed=await read();livePauseProof={completed:true,keyboardEvents:true,paused:{frame:paused.frames,match:paused.match},held:{frame:held.frames,match:held.match},resumed:{frame:resumed.frames,match:resumed.match},identicalFrozenFighterStates:true};
    }
    if(liveController){
      liveControllerProof=[];
  ...
  if(livePause&&!probe.error){if(!livePauseProof?.completed)throw Error('Incomplete live pause input');probe.livePause=livePauseProof;}
  ...
  if(kirbyCopy&&renderSteps&&!probe.error&&[3,15,16,22,24].includes(probe.kirbyCopy.copyKind)&&!(probe.kirbyCopy.bodyRenderedFrames>0&&probe.kirbyCopy.peakBodyDraws>0))throw Error('Copied body was not drawn');
  if(kirbyCopy&&renderSteps&&!probe.error&&probe.kirbyCopy.copyKind===10&&!(probe.kirbyCopy.hammerRenderedFrames>0&&probe.kirbyCopy.peakHammerDraws>0))throw Error('Copied hammer was not drawn');
  if(kirbyCopy&&renderSteps&&!probe.error&&probe.kirbyCopy.eggAccessoryFrames&&!(probe.kirbyCopy.eggRenderedFrames>0&&probe.kirbyCopy.peakEggDraws>0))throw Error('Copied egg shell was not drawn');
  ...
  if(kirbyCopy&&renderSteps&&!probe.error)for(const name of [...(kirbyCopy==='spit'?['captured']:kirbyCopy==='reflect-control'?['copyhat','captured']:['acquire','contact','reflect'].includes(kirbyCopy)?['copyhat',probe.kirbyCopy.projectileKind===null?'copyattack':'copyfire','captured']:['copyhat',probe.kirbyCopy.projectileKind===null?'copyattack':'copyfire','copystar','captured']),...(kirbyCopy==='reflect'?['copyreflected']:[]),...(probe.kirbyCopy.copyKind===14&&probe.kirbyCopy.eggAccessoryFrames?['copyegg']:[]),...(probe.kirbyCopy.itemKinds.includes(probe.kirbyCopy.secondaryItemKind)?['copysecondary']:[])]){
    ...
    fs.writeFileSync(path.join(output,'Kb-native-'+name+'.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
  if(kirbyMoves&&!probe.error&&!probe.kirbyMoves?.completed)throw Error('Incomplete Kirby moves');
  if(kirbyMoves&&renderSteps&&!probe.error)for(const name of kirbyMove?['cutter']:['hammer','cutter','stone']){
    if(!probe.preview?.[name])throw Error('Missing rendered Kirby '+name);
    const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-"+name+"').src",returnByValue:true});
    if(!shot.result.value?.startsWith('data:image/png;base64,'))throw Error('Missing Kirby screenshot');
    fs.writeFileSync(path.join(output,'Kb-native-'+name+'.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
  if(climbersMoves&&!probe.error&&!probe.climbersMoves?.completed)throw Error('Incomplete climbers moves');
  if(climbersMoves&&renderSteps&&!probe.error){
    if(!probe.preview?.belay)throw Error('Missing rendered Belay');
    const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-belay').src",returnByValue:true});
    if(!shot.result.value?.startsWith('data:image/png;base64,'))throw Error('Missing Belay screenshot');
    fs.writeFileSync(path.join(output,'Pp-native-belay.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
  }
  ...
  if(pauseInput&&renderSteps&&!probe.error){
    for(const name of ['pause0','pausecamera0','resume0','pause1','pausecamera1','resume1']){
      const draws=probe.preview[name]?.hud?.objects.find(o=>o.name==='native pause')?.draws;
      if(name.startsWith('resume')?draws!==0:!(draws>0))throw Error('Native pause artwork visibility mismatch '+name);
      const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-"+name+"').src",returnByValue:true});
      if(!shot.result.value?.startsWith('data:image/png;base64,'))throw Error('Missing native pause screenshot '+name);
      fs.writeFileSync(path.join(output,'native-'+name+'.png'),Buffer.from(shot.result.value.split(',')[1],'base64'));
    }
  }
  const rendererSources=Object.fromEntries(['model-geometry-cache.mjs','native-input.mjs','material-gpu.mjs','material-shader.mjs','tev-shader.mjs','immediate-geometry.mjs','native-live.mjs','native-pixel.mjs','native-match-preview.mjs','combat-workload.mjs'].map(name=>[name,createHash('sha256').update(fs.readFileSync(path.join(output,name))).digest('hex')]));
  if(illusionAttachments&&!probe.error&&!probe.illusionAttachments?.completed)throw Error('Incomplete Illusion attachment probe');
  const report={illusionAttachments,costume,opponentCostume,livePause,pauseInput,cacheModels,controllerInput,liveController,consoleDiagnosticCount,consoleDiagnosticTail:diagnostics.slice(-8),validationTimeoutSeconds:probeSeconds,stage:stageKey,kirbyCopy,kirbyMoves,kirbyMove,climbersMoves,climbersMove,peachMoves,peachPulls,peachContact,character,opponent,transform,liveTransform,formMoves,formMove,formContact,absorption,nessMoves,nessContact,gamewatchMoves,gamewatchContact,mewtwoMoves,mewtwoContact,yoshiMoves,yoshiContact,linkMoves,linkContact,koopaContact,koopaMoves,samusContact,samusMoves,pikachuMoves,marioMoves,purinMoves,purinContact,stadiumFireworksOff,fountainCosmeticsOff,fountainSceneryOff,recordShaders,prewarmShaders,deferredGpuErrors,driverShaderCacheDisabled:process.env.MESA_SHADER_CACHE_DISABLE==='true',rendererSources,cpuProfileInstrumented:cpuProfile,gpuRequested:hardware?'hardware':'software',browser:execFileSync(chrome,['--version'],{encoding:'utf8'}).trim(),build:JSON.parse(fs.readFileSync(path.join(output,'fighter-init-build.json'))),probe};
