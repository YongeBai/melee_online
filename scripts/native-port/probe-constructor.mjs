...
try {
  ...
  for(const name of ['yoyo','pkflash','pkthunder','gwchef','gwjudge','gwrescue','gwturtle','shadowball','disable','teleport','capturedegg','eggshield','eggthrow','eggroll','yoshistars','linkhit','arrow','hookshot','boomerang','flame','claw','throw','charge','missile','grapple','jolt','thunder'])if(probe.preview?.[name]&&!probe.error){
    const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-"+name+"').src",returnByValue:true});
