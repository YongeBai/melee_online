...
try {
  ...
  if(recordShaders&&!probe.error){
    ...
    catalog.recording={stage:stageKey,character,fountainCosmeticsOff,fountainSceneryOff,frames:probe.live?.frames??probe.input?.frames??probe.combat?.frames??0,stageCallbackFrames:probe.stage?.callbackFrames??0,workload,stageOnly,inheritedPrograms:probe.shaderPreparation?.compiled??0};
