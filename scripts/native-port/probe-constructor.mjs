...
try {
  ...
  for(const name of ['jolt','thunder'])if(probe.preview?.[name]&&!probe.error){
    const shot=await command('Runtime.evaluate',{expression:"document.getElementById('native-preview-"+name+"').src",returnByValue:true});
  ...
  const report={stage:stageKey,character,pikachuMoves,marioMoves,purinMoves,purinContact,stadiumFireworksOff,fountainCosmeticsOff,fountainSceneryOff,recordShaders,prewarmShaders,deferredGpuErrors,driverShaderCacheDisabled:process.env.MESA_SHADER_CACHE_DISABLE==='true',rendererSources,cpuProfileInstrumented:cpuProfile,gpuRequested:hardware?'hardware':'software',browser:execFileSync(chrome,['--version'],{encoding:'utf8'}).trim(),build:JSON.parse(fs.readFileSync(path.join(output,'fighter-init-build.json'))),probe};
