...
try{
 ...
 fs.writeFileSync(output+'/raw-report.json',JSON.stringify({requested:{pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays,clientDelays},initial,final,transport,receiveTransport},null,2));
 ...
 const scope=captureMode?'Two localhost browser processes using authenticated local prediction, complete-state correction and independent WASM presentation; canvas-capture observation on seat(s) '+[...captureSeats].join(',')+(combatMode?' under scripted combat.':'.')+(relayDelays?' Ordered relay-to-client input/confirmation delay was injected.':'')+(clientDelays?' Ordered client-to-relay input delay was injected.':'')+(relayDelays||clientDelays?' This is not full WAN emulation.':'')+' Not physical presentation, WAN, input-to-photon, roster or tournament certification.':rollbackMode?'Two localhost browser processes using authenticated local prediction, complete-state correction and independent WASM presentation; bounded correctness probe, not 720p60 or WAN certification.':'Two localhost browser processes, three-frame input lockstep; not WAN latency or rollback certification.';fs.writeFileSync(output+'/report.json',JSON.stringify({passed:true,requested:{pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays,clientDelays},initial,final,transport,receiveTransport,scope},null,2));console.log(JSON.stringify({passed:true,code,frames:matchFrames,rollback:rollbackMode,combat:combatMode,pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays,clientDelays,captureSeats:[...captureSeats],matchingFighterState:true}));
}
