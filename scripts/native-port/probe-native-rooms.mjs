...
if(relayDelays&&!rollbackMode)throw Error('Relay delay injection requires rollback mode');
let relayDelayIndex=0;const server=createNativePortServer({roomOptions:relayDelays?{deliveryDelayMs:m=>m.key?.startsWith('match:')&&m.frame>=3&&['peer-input','confirmed-frame'].includes(m.type)?relayDelays[relayDelayIndex++%relayDelays.length]:null}:undefined});
...
async function client(capture=false){
 ...
 await c.cmd('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+'/character-menu.html?interactive=1'+(resultsMode?'':'&liveframes='+matchFrames+(rollbackMode?'&rollback=1':'')+(capture?'&captureframes=1':'')+(combatMode?'&workload=1':''))});await c.wait('globalThis.characterMenuReport?.passed');return c;
}
try{
 ...
 const final=await Promise.all([a,b].map(c=>c.eval('({room:nativeRoom.snapshot(),report:nativeMenuMatchReport})'))),transport=server.nativeRoomRelay?.deliverySnapshot()??null;
 fs.writeFileSync(output+'/raw-report.json',JSON.stringify({requested:{pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays},initial,final,transport},null,2));
 ...
 const captureSummaries=final.map(v=>{const live=v.report.live,o=live.observation;return {frames:live.frames,draws:live.draws,simulationFps:live.frames*1000/live.elapsedMs,drawSubmissionCpu:live.drawSubmissionCpu,observation:o&&{enabled:o.enabled,mode:o.mode,sampleRegion:o.sampleRegion,requested:o.requested,captured:o.captured,estimatedUnobservedRequests:o.estimatedUnobservedRequests,distinctSampledImages:o.distinctSampledImages,repeatedSampledImages:o.repeatedSampledImages,blackFrames:o.blackFrames,wrongSize:o.wrongSize,cadence:o.cadence,observerWorkerCpu:o.observerWorkerCpu,error:o.error}};});
 ...
 const scope=captureMode?'Two localhost browser processes using authenticated local prediction, complete-state correction and independent WASM presentation; canvas-capture observation on seat(s) '+[...captureSeats].join(',')+(combatMode?' under scripted combat.':'.')+(relayDelays?' Ordered relay-to-client input/confirmation delay was injected; this is not full WAN emulation.':'')+' Not physical presentation, WAN, input-to-photon, roster or tournament certification.':rollbackMode?'Two localhost browser processes using authenticated local prediction, complete-state correction and independent WASM presentation; bounded correctness probe, not 720p60 or WAN certification.':'Two localhost browser processes, three-frame input lockstep; not WAN latency or rollback certification.';fs.writeFileSync(output+'/report.json',JSON.stringify({passed:true,requested:{pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays},initial,final,transport,scope},null,2));console.log(JSON.stringify({passed:true,code,frames:matchFrames,rollback:rollbackMode,combat:combatMode,pair,stage:selectedStage,holdASeat,clientCpuSets,relayDelays,captureSeats:[...captureSeats],matchingFighterState:true}));
}
