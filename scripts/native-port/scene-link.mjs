...
export function sceneLinkInputs(root,upstream,output,additionalFiles=[]) {
  ...
  const tevSource=path.join(root,'engines/browser-native/tev-state.c'),textureSource=path.join(root,'engines/browser-native/texture-state.c');
  const tevFunctions=new Set([...(fs.readFileSync(tevSource,'utf8')+fs.readFileSync(textureSource,'utf8')).matchAll(/\bvoid (GX\w+)\(/g)].map(m=>m[1]));
  ...
  return {files:[path.join(root,'engines/browser-native/scene.c'),tevSource,textureSource,guards,library,...characterObjects],
