...
export function commandProbeSource(fields) {
  const expression=f=>f.view==='fighterAnim'?`fighter.${f.path}`:`((${({command:'CmdUnion',dispatch:'gmScriptEventDefault',motion:'struct ftData_80085FD4_ret',color:'union ColorOverlay_x8_t',skip:'struct spawn_hitbox_skip'})[f.view]}*)words)->${f.path}`;
