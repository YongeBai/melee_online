...
export function summarizeDrawProfile(profile){
 ...
 const self=new Map(),inclusive=new Map(),stages={nativeTraversalAndCapture:0,uniformsAndTextures:0,otherFlush:0,otherDraw:0};let samples=0,drawMs=0,totalMs=0;const outsideDraw={garbageCollectorMs:0,idleMs:0,otherMs:0};
 for(let i=0;i<profile.samples.length;i++){
  ...
  const stage=has('material-gpu.mjs','apply')?'uniformsAndTextures':has('material-gpu.mjs','flush')?'otherFlush':stack.some(n=>n.callFrame.url.includes('wasm:'))?'nativeTraversalAndCapture':'otherDraw';stages[stage]+=dt;
 }
 ...
 const functions=['generateMaterialShaders','materialShaderKey','packRows','equalTextureBytes'];
 ...
 return {scope:'CPU sampling under timed drawFrame only; includes native traversal/capture and WebGL submission, excludes final pixel oracle. Inclusive rows overlap. Not GPU elapsed time; profiling adds overhead.',totalProfileMs:totalMs,outsideDraw,drawSamples:samples,sampledDrawMs:drawMs,stages,sourceSelfMs:Object.fromEntries([...sourceSelf].sort((a,b)=>b[1]-a[1])),functionSelfMs,self:rows(self),inclusive:rows(inclusive)};
}
