...
function fixture(external=300) {
  const size=320,relocs=[8,92,164,172,184,204,212,220,264],text=new TextEncoder().encode('ftDataMario\0unused\0');
  ...
  d.setUint32(168,1);d.setUint32(172,264);d.setUint32(264,268);d.setUint16(268,9);d.setUint32(160,1);d.setUint32(164,184);d.setUint32(184,200);
...
test('visibility import preserves index bytes and only exposes its typed graph',()=>{
  ...
  assert.deepEqual(a.rows,[[[[[0,123],[4]]],null,null,null]]);assert.equal(a.models,1);assert.deepEqual(a.textureRows,[[9]]);
  assert.deepEqual(a.image.subarray(288,290),Uint8Array.of(0,123));assert.equal(v.getUint32(32+200,true),2);
  assert.equal(v.getUint32(8,true),7);assert.deepEqual(f.bytes,before);
});
