...
const gameInputs=fighterInit?gameLinkInputs(root,upstream,output,{implementedPlatform:resultEfbFunctions}):null;
...
execFileSync(compiler, [...flags, ...selectedUnits.map(file=>path.join(portable.directory,file)), selectedSdk,sdkLight,sdkProject,...(scene?[sdkFog]:[]),sdkTextureSize,sdkTevClamp,textureSource,...estimateObjects,
  ...
  ...(gameInputs?[path.join(root,'engines/browser-native/result-scene-config.c'),path.join(root,'engines/browser-native/result-efb.c')]:[]),
