...
if(fighterInit)exports.push('gm_80168B34','portTournamentDamageInitialize','portHudPlayerRead','portTournamentHudInitialize','portHudObjects','portHudRenderBegin','portHudCameraSnapshot','portTournamentStatusInstall','portTournamentInitialize','portTournamentBegin','portTournamentStep','portTournamentRead','portStageRenderInitialize','portStageRenderBegin','portStageCallbackBits','portStageCameraStart','portStageCameraSnapshot','portProbeRulesInitialize','Player_80031848','portSceneInitialize','portStageMapInstall','portStageMapCollisionRead','portStageProbePad','portStageMapCollisionLoad','portStageMapSpawn','portStageMapCreate','portStageMapBounds','portStageMapRead','portStageMapLights','portStageMapClear','portStageLightOverrideBits','portMatchCameraInitialize','portParticleOperandBits','portEffectsInitialize','portEffectsLoad','portCommonEffectsLoad','portEffectsRead','portEffectCreate','portEffectBankCreate','portEffectsDestroyOwner','portEffectParentCreate','portEffectLife','portEffectStep','portEffectsParticleStep','portMatchPlayerInitialize','portPackedFlagBits','portFighterPreviewPrepare','portFighterPreviewFinish','portFighterNativeDraw','portFighterRespawnPlatform','portFighterConstruct','portFighterConstructRead','portItemModelCreate','portItemAttrFlags','portItemModelTransform','portItemModelRead','portOriginalItemModelLive','portFighterMotionRegister','portFighterMotionUnregister','portFighterMotionAttach','portFighterMotionLoad','portFighterMotionRead','portSecondaryAttach','portPartAnimationApply','portPartAnimationStep','portPartAnimationClear','portPartAnimationRead','portShieldPoseApply','portSecondaryJointRead','portGameplayAttach','portGameplayRescale','portGameplayUpdate','portGameplayRead','portFighterAnimationInitialize','portFighterAnimationStart','portFighterAnimationStep','portFighterAnimationRead','portDynamicsInitialize','portDynamicsPoolFree','portDynamicsAttach','portDynamicsRead','portDynamicsStep','portDynamicsSelect','Fighter_Create','portFighterInitModelCreate','portFighterPlayerConfigure','portFighterInitRead');
...
execFileSync(compiler, [...flags, ...selectedUnits.map(file=>path.join(portable.directory,file)), selectedSdk,sdkLight,sdkProject,textureSource,...estimateObjects,
  ...
  ...(gameInputs?[path.join(root,'engines/browser-native/match.c'),path.join(root,'engines/browser-native/stage-map.c'),path.join(root,'engines/browser-native/item-model.c'),path.join(root,'engines/browser-native/effects.c'),...gameInputs.files]:[]),
