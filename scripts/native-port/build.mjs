...
const resultScene=process.argv.includes('--result-scene');
const fighterInit=resultScene||process.argv.includes('--fighter-init');
const startup=fighterInit||process.argv.includes('--startup');
if(['--scene','--startup','--fighter-init','--result-scene'].filter(flag=>process.argv.includes(flag)).length>1)throw Error('Choose one native integration target');
const scene=startup||process.argv.includes('--scene'),moduleName=resultScene?'melee-result-scene':fighterInit?'melee-fighter-init':startup?'melee-startup':scene?'melee-scene':'melee-native';
...
// Original fog coefficient/register packing with host register submission.
const sdkPixelText=fs.readFileSync(path.join(upstream,'libs/dolphin/src/dolphin/gx/GXPixel.c'),'utf8');
const fogStart='void GXSetFog(',fogEnd='\nvoid GXInitFogAdjTable(';
if(sdkPixelText.split(fogStart).length!==2||sdkPixelText.split(fogEnd).length!==2)throw Error('SDK fog selection changed');
const sdkFog=path.join(output,'sdk-fog.c');
fs.writeFileSync(sdkFog,'#include '+JSON.stringify(path.join(upstream,'libs/dolphin/src/dolphin/gx/__gx.h'))+'\n#include <macros.h>\n#include <dolphin/os.h>\nvoid portFogRegister(u32);\n#undef GX_WRITE_RAS_REG\n#define GX_WRITE_RAS_REG(x) portFogRegister(x)\n'+sdkPixelText.slice(sdkPixelText.indexOf(fogStart),sdkPixelText.indexOf(fogEnd)).replace('void GXSetFog(','void portSdkSetFog(').replace('    gx->bpSent = 0;','    /* Host register capture has no GX command FIFO. */'));
if(scene)selectedSdkFunctions.push('GXSetFog');
...
const resultEfbFunctions=['GXCopyTex','GXInvalidateTexAll','GXSetTexCopyDst','GXSetTexCopySrc'];
const gameInputs=fighterInit?gameLinkInputs(root,upstream,output,{implementedPlatform:resultScene?resultEfbFunctions:[]}):null;
const sceneInputs=scene?sceneLinkInputs(root,upstream,output,attributeSpec.characters.map(c=>c.file),Object.keys(gameInputs?.replacedDefinitions??{})):null;
if(scene) {
  ...
  exports.push('portCostumeString','portFighterModelLive','portFighterModelCreate','portCostumeLoad','portCostumeAnimation','portCostumeRelease','portMaterialColorSelect','portMaterialColorRead','portMaterialAttach','portMaterialSelect','portMaterialReset','portMaterialRead','portCostumeCount','portVisibilityAttach','portVisibilitySelect','portVisibilityApply','portVisibilityRead','portCollisionAuxiliary','portCollisionAuxiliaryRead','portLightColor','GXInitLightSpot','GXInitLightDistAttn','GXInitLightPos','GXInitLightDir','GXInitLightColor','GXGetLightColor','portCollisionPartRead','portAttributeField','portAttributeSize','portAttributesLoad','portCollisionAttach','portCollisionReset','portCollisionWorld','portCollisionRead','portSharedInitialize','portSharedGlobal','portSceneJointAnimation','portCpuScript','portCpuChoose','portColorCreate','portColorDestroy','portColorSelect','portColorStep','portColorRead','portSharedField','portSharedPart','portSharedLanding','portMotionCreate','portMotionDestroy','portMotionLoad','portMotionEntry','portMotionLive','portMotionBuffers','portFilePins',
    ...
    'portSceneAnimation','portSceneRequest','portSceneAnimate','portSceneFlags','portSceneMeshVisibility','portMaterialTev','portMaterialTextureState','portMaterialPixelState','portMaterialDrawState','GXProject','portMaterialPolygon','portMaterialPolygonDescriptor','portNativeDrawObject','portMaterialModelState','portFogState','portRenderContextState','portSceneLiveObjects','portSceneLiveMetric');
}
if(fighterInit)exports.push('portTournamentFinish','portTournamentResultRead','portMenuRestorePlayer','portStageMenuRematch','portMenuSetCPU','portTapJumpSet','portTapJumpGet','portCharacterMenuHand','portMenuProduct','portMenuConfigure','portTournamentInitializeMenu','portTournamentConstructSelected','portTournamentInitializeCostumes','portStageMenuFromCharacters','portCharacterMenuResume','portMenuMatchRead','portControllerDisconnect','portCharacterMenuFinish','portCharacterMenuNativeRead','portCharacterMenuObjects','portCharacterMenuRenderBegin','portCharacterMenuCameraSnapshot','portCharacterMenuStep','portCharacterMenuRead','portMenuDiagnosticMute','portCharacterMenuInitialize','portTextureDefaultState','portTextInitialize','portTextCreate','portTextCreateString','portTextDestroy','portTextFinish','portTextOwner','portTextStream','portTextMeasure','portTextStyleStackProbe','portNativeDrawText','portShapeBlend','portStageMenuFinish','portStageMenuNativeRead','portStageMenuObjects','portStageMenuRenderBegin','portStageMenuCameraSnapshot','portStageMenuInitialize','portStageMenuStep','portStageMenuRead','ftKb_SpecialN_800EED50','portKirbyCopyRoot','portKirbyCostumeRoot','portKirbyCostumeRootAt','portKirbyCostumeString','portKirbyBodyNodes','portKirbyRead','portKirbyFlameRead','Player_GetEntity','Player_GetEntityAtIndex','portStadiumFireworksOff','portStadiumRead','portRandallRead','portStoryItemInstall','portCommonItemInstall','portStageParticlesInstall','portDreamlandWindRead','portFountainPlatformRead','portFountainReflectionOff','portStageObjects','portStageAnimationProbe','portStageShadowBits','portTournamentInitializeStage','portStageMapInstallKind','portStageCallbacksInitialize','portStageObject','gm_80168B34','portTournamentDamageInitialize','portHudPlayerRead','portTournamentPauseInitialize','portTournamentPauseRead','portTournamentHudInitialize','portHudObjects','portHudRenderBegin','portHudCameraSnapshot','portTournamentStatusInstall','portTournamentInitialize','portTournamentInitializeKinds','portTournamentBegin','portTournamentIntroBegin','portTournamentStep','portTournamentRead','portStageRenderInitialize','portStageRenderBegin','portStageCallbackBits','portStageCameraStart','portStageCameraSnapshot','portProbeRulesInitialize','Player_80031848','portSceneInitialize','portStageMapInstall','portStageMapCollisionRead','portControllerSample','portStageProbePad','portStageMapCollisionLoad','portStageMapSpawn','portStageMapCreate','portStageDrawPasses','portBattlefieldCallbacksInitialize','portBattlefieldObject','portStageMapBounds','portStageMapRead','portStageMapLights','portStageMapClear','portStageLightOverrideBits','portMatchCameraInitialize','portParticleOperandBits','portEffectsInitialize','portNativeDrawParticles','portEffectModels','portEffectsLoad','portEffectsBankData','portCommonEffectsLoad','portEffectsRead','portEffectCreate','portEffectBankCreate','portEffectsDestroyOwner','portEffectParentCreate','portEffectLife','portEffectStep','portEffectsParticleStep','portMatchPlayerInitialize','portPackedFlagBits','portFighterPreviewPrepare','portFighterPreviewFinish','portFighterNativeDraw','portFighterRespawnPlatform','portFighterAccessory','portFighterConstruct','portFighterConstructRead','portItemsInitialize','portItemsList','portItemLinksList','portItemAttachmentsList','portItemRead','portItemCommonProbe','portArticleProbeCreate','portArticleProbeStart','portArticleProbeStep','portArticleProbeRead','portItemModelCreate','portItemAttrFlags','portItemModelTransform','portItemModelRead','portOriginalItemModelLive','portFighterMotionRegister','portFighterMotionUnregister','portFighterMotionAttach','portFighterMotionLoad','portFighterMotionRead','portSecondaryAttach','portPartAnimationApply','portPartAnimationStep','portPartAnimationClear','portPartAnimationRead','portShieldPoseApply','portSecondaryJointRead','portGameplayAttach','portGameplayRescale','portGameplayUpdate','portGameplayRead','portFighterAnimationInitialize','portFighterAnimationStart','portFighterAnimationStep','portFighterAnimationRead','portDynamicsInitialize','portDynamicsPoolFree','portDynamicsAttach','portDynamicsRead','portDynamicsStep','portDynamicsSelect','Fighter_Create','portFighterInitModelCreate','portFighterPlayerConfigure','portFighterInitRead');
if(fighterInit)exports.push('portResultSceneInitialize','portResultSceneObjects','portResultSceneStep','portResultSceneRenderBegin','portResultSceneCameraSnapshot','portResultSceneFinish');
if(resultScene)exports.push('portResultScenePlayerSnapshot','portResultSceneWinnerSnapshot','portResultSceneConfigureStats','portResultSceneTextOwners','portResultFightersInitialize','portResultFighter','portResultFighterSnapshot','portResultFighterCamera','portResultFighterCameraSnapshot','portResultFighterCameraProjectionSnapshot','portResultFighterRenderBegin','portResultFighterNativeDraw','portResultWinnerCameraSubmission','portResultPortraitDescriptor','portResultPortraitCopy','portResultPortraitAttach','portResultPortraitAttachment','portEfbCopyCount','portItemsInitialize');
...
execFileSync(compiler, [...flags, ...selectedUnits.map(file=>path.join(portable.directory,file)), selectedSdk,sdkLight,sdkProject,...(scene?[sdkFog]:[]),sdkTextureSize,sdkTevClamp,textureSource,...estimateObjects,
  ...
  ...(gameInputs?[path.join(root,'engines/browser-native',resultScene?'result-scene-config.c':'result-scene.c'),...(resultScene?[path.join(root,'engines/browser-native/result-efb.c')]:[])]:[]),
  ...(gameInputs?[path.join(root,'engines/browser-native/music.c'),path.join(root,'engines/browser-native/menu-audio.c'),path.join(root,'engines/browser-native/menu.c'),path.join(root,'engines/browser-native/tap-jump.c'),path.join(root,'engines/browser-native/text.c'),path.join(root,'engines/browser-native/match.c'),path.join(root,'engines/browser-native/stage-map.c'),path.join(root,'engines/browser-native/item-model.c'),path.join(root,'engines/browser-native/item-runtime.c'),path.join(root,'engines/browser-native/effects.c'),...gameInputs.files]:[]),
...
for (const name of ['stage-archive.mjs','stage-item-assets.mjs','common-item-assets.mjs','shader-catalog.mjs','articles.html','item-runtime-assets.mjs','item-common-assets.mjs','article-assets.mjs','verify-articles.mjs','stage-map-assets.mjs','verify-stage-map.mjs','stage-map.html','effect-assets.mjs','verify-effects.mjs','tev-shader.mjs','tev-reference.mjs','verify-tev.mjs','tev-check.html','native-tev.mjs','native-texture.mjs','native-pixel.mjs','native-model.mjs','native-render-context.mjs','material-shader.mjs','material-gpu.mjs','owned-uniform-state.mjs','draw-uniform-buffer.mjs','native-state-intern.mjs','verify-material-shader.mjs','verify-model-state.mjs','verify-material-state.mjs','model-geometry-cache.mjs','native-match-preview.mjs','immediate-geometry.mjs','snapshot-page-kernel.mjs','paged-snapshot.mjs','presentation-cache.mjs','wasm-snapshot.mjs','rollback-session.mjs','rollback-audio.mjs','dirty-runtime.mjs','dirty-host-contract.mjs','render-replica.mjs','frame-evidence-worker.mjs','frame-evidence.mjs','certification.html','certification.mjs','rollback-probe.html','rollback-probe.mjs','native-results.mjs','native-results-ui.mjs','native-room.mjs','native-music.mjs','native-music-worker.mjs','hps-audio.mjs','native-product-ui.mjs','native-product.css','browser-input.mjs','native-menu-live.mjs','native-input.mjs','verify-controller.mjs','native-live.mjs','combat-workload.mjs','native-camera.mjs','verify-combat.mjs','verify-match.mjs','verify-intro.mjs','player-parameters.mjs','character-menu.html','card-assets.mjs','sis.html','sis-assets.mjs','menu.html','native-character-menu-preview.mjs','native-menu-preview.mjs','menu-assets.mjs','shape-assets.mjs','shape-geometry.mjs','verify-shape.mjs','pause-assets.mjs','verify-pause.mjs','status-assets.mjs','constructor.html','constructor-runner.mjs','fighter-base-assets.mjs','fighter-package.mjs','kirby-copy-assets.mjs','verify-purin.mjs','verify-pikachu.mjs','verify-samus.mjs','verify-koopa.mjs','verify-link.mjs','verify-yoshi.mjs','verify-mewtwo.mjs','verify-gamewatch.mjs','verify-ness.mjs','verify-absorption.mjs','verify-transform.mjs','verify-seak-zelda.mjs','verify-peach.mjs','verify-climbers.mjs','verify-kirby.mjs','verify-mario-family.mjs','purin-extra-assets.mjs','fox-extra-assets.mjs','item-model-assets.mjs','verify-item-models.mjs','secondary-animation-assets.mjs','verify-secondary-animation.mjs','gameplay-assets.mjs','verify-gameplay.mjs','verify-fighter-animation.mjs','dynamics-assets.mjs','verify-dynamics.mjs','fighter-init-assets.mjs','verify-fighter-init.mjs','fighter-init.html','startup.html','verify-startup.mjs','costumes.html','costume-assets.mjs','animation-object-assets.mjs','material-animation-assets.mjs','verify-material-animation.mjs','visibility-assets.mjs','auxiliary-assets.mjs','verify-lights.mjs','attribute-assets.mjs','verify-attributes.mjs','character-collision-assets.mjs','verify-character-collision.mjs','verify-common-initialization.mjs','joint-animation-assets.mjs','verify-cpu.mjs','cpu-assets.mjs','color-reference.mjs','verify-colors.mjs','color-assets.mjs','verify-shared.mjs','shared-assets.mjs','verify-motions.mjs','motion-assets.mjs','motion-animations.mjs','verify-commands.mjs','resident-files.mjs','verify-resident-files.mjs','archive.mjs','scene-assets.mjs','verify-scene.mjs','scene.html', 'stage-collision.mjs', 'fighter-assets.mjs', 'verify-fighters.mjs',
 'animation-assets.mjs','result-motion-assets.mjs','verify-animations.mjs','math-reference.mjs','verify-math.mjs',
...
for(const name of ['native-result-scene-preview.mjs','result-efb.mjs','result-scene.html'])
  fs.copyFileSync(path.join(root,'engines/browser-native',name),path.join(output,name));
...
fs.writeFileSync(path.join(output, resultScene?'result-scene-build.json':fighterInit?'fighter-init-build.json':startup?'startup-build.json':scene?'scene-build.json':'build.json'), JSON.stringify(report, null, 2) + '\n');
