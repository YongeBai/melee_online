...
export function gameLinkInputs(root,upstream,output) {
  ...
  const definitions=names.map(name=>{
    ...
    const candidates=[...new Set([...headers.matchAll(pattern)].map(m=>m[1].replace(/\s+/g,' ').trim()))];
  ...
  return {files:[...sources,guards,fontSource,library],unavailablePlatform:names,fontBytes:font.length,fontSha256:createHash('sha256').update(font).digest('hex'),replacedDefinitions:replacements,linkedAuditObjects:objects.length};
}
