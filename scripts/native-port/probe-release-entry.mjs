...
const accessKey=randomBytes(24).toString('hex'),{server,config}=createReleaseServer({MELEE_ACCESS_KEY:accessKey}),profile=fs.mkdtempSync(path.join(os.tmpdir(),'native-release-entry-')),pending=new Map(),events=[];let browser,ws,id=0,stderr='';
...
try{
 ...
 config.origins.splice(0,config.origins.length,base);
 if((await fetch(base+'/play/')).status!==401)throw Error('Unauthenticated release entry');
 const login=await fetch(base+'/health',{headers:{Authorization:'Basic '+Buffer.from('player:'+accessKey).toString('base64')}}),cookie=login.headers.get('set-cookie').split(';')[0],headers={cookie};
 const redirect=await fetch(base+'/?join=ABC234',{headers,redirect:'manual'});if(redirect.headers.get('location')!=='/play/?interactive=1&join=ABC234')throw Error('Release redirect lost its invite');
 const restricted=await fetch(base+'/play/?diagnostic-cpu=1&lockstep=1&workload=1&liveframes=60&captureframes=1',{headers,redirect:'manual'});if(restricted.status!==302||restricted.headers.get('location')!=='/play/?interactive=1')throw Error('Release route exposed diagnostics');
 for(const route of ['/play/certification.html','/local-disc','/game.iso'])if((await fetch(base+route,{headers})).status!==404)throw Error('Release route escaped product boundary: '+route);
 const post=body=>fetch(base+'/native-rooms',{method:'POST',headers:{...headers,Origin:base,'Content-Type':'application/json'},body:JSON.stringify(body)});
 if((await post({diagnosticCpu:true})).status!==400)throw Error('Release API exposed CPU rooms');
 if((await fetch(base+'/native-rooms',{method:'POST',headers:{...headers,Origin:'https://wrong.test'},body:'{}'})).status!==403)throw Error('Release API accepted foreign origin');
 const health=await login.json();if(health.engine!=='browser-native-wasm'||health.dolphin!==false||health.width!==960||health.height!==720)throw Error('Invalid release health '+JSON.stringify(health));
 ...
 await command('Network.setCookie',{name:'melee_session',value:cookie.slice('melee_session='.length),url:base,httpOnly:true,sameSite:'Strict'});
