...
try{
 ...
 const redirect=await fetch(base+'/?join=ABC234',{redirect:'manual'});if(redirect.headers.get('location')!=='/play/?join=ABC234')throw Error('Release redirect lost its query');
 ...
 await command('Runtime.enable');await command('Log.enable');await command('Page.navigate',{url:base+'/play/?interactive=1&liveframes=60'});await waitFor('globalThis.characterMenuReport?.passed&&globalThis.nativeCharacterMenu?.read().frames>30');
 const state=await evaluate(`(()=>{const canvas=document.querySelector('#picture'),room=nativeRoom.snapshot(),menu=nativeCharacterMenu.read();return {path:location.pathname,canvas:[canvas.width,canvas.height],scene:nativeMenuLive.snapshot().scene,room:{cpu:nativeRoom.cpu,active:nativeRoom.active,mode:room.mode,code:room.code},p2Kind:menu.players[1].kind,fileInputs:document.querySelectorAll('input[type=file]').length};})()`);
 if(state.path!=='/play/'||state.canvas[0]!==960||state.canvas[1]!==720||state.scene!=='characters'||state.room.cpu!==false||state.room.active!==false||state.room.mode!=='solo'||!state.room.code||state.p2Kind!==0||state.fileInputs)throw Error('Hosted entry invariant failed '+JSON.stringify(state));
