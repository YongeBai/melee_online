...
test('static background comparison uses both stage-specific directions and restores animation',async()=>{
 ...
 const host={adapter:{request:async(_type,data)=>{calls.push(data);if(data.action==='staticBackgroundAnimation'){enabled=data.enabled;return{objects:[{mapId:1}],writes:[],codeWrites:[],preserved:['main Battlefield','three platforms']};}return{};}}};
