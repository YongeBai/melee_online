...
export async function compareBrowserRushPresentation(host, seconds, inspect, {
  ...
  try {
    await command('pause');original=(await command('rushPresentation')).enabled;
    ...
    for(const[index,enabled]of[true,false,false,true].entries())for(const warmup of[true,false]) {
      const label=`Rush presentation ${enabled?'on':'off'} ${warmup?'warmup':'measurement'} ${index+1}/4`;
      onProgress(label+': restore and settle');
      await command('pause');await command('restore',{slot:5});
      if((await command('rushPresentation',{enabled})).enabled!==enabled)throw Error('Timing mode did not apply');
      for(let frame=0;frame<120;frame++)await command('step');
      await command('frameInput',{enabled:true});
      const before=await command('rushPresentation');
      await host.adapter.request('start',{});
      const measured=await measure(host,seconds,inspect,{onPhase:phase=>onProgress(label+': '+phase)});
      await command('pause');const after=await command('rushPresentation');
      if(before.enabled!==enabled||after.enabled!==enabled)throw Error('Timing mode changed during capture');
      const calls=after.calls-before.calls,skips=after.skips-before.skips;
      if(!Number.isSafeInteger(calls)||!Number.isSafeInteger(skips)||calls<=0||skips<0||skips>calls||!enabled&&skips!==0)throw Error('Rush presentation counters are inconsistent');
      ...
      runs.push({...measured,enabled,warmup,rushPresentation:{before,after,calls,skips,
        scope:'Paused diagnostic boundaries bracket the image interval. Host throttle calls and calls skipped by the existing rush policy. No skipped game instructions.'},
        controllerStress:{...input,kind:'two native-frame human controller tracks',notHumanPlay:true,exercisedBothPlayers:true}});
      onResult({kind:'same-checkpoint-rush-presentation-abba',passed:false,runs});
    }
    const inputConsistency=compareFrameInputDigests(runs);
    result={kind:'same-checkpoint-rush-presentation-abba',passed:inputConsistency.passed&&runs.filter(r=>!r.warmup&&!r.enabled).every(r=>r.passed),runs,inputConsistency};
  }catch(error){failed=error;}
  const errors=[];
  for(const cleanup of[()=>command('pause'),()=>command('frameInput',{enabled:false}),
    ()=>captured?command('release',{slot:5}):null,
    ()=>typeof original==='boolean'?command('rushPresentation',{enabled:original}):null,
...
export async function verifyBrowserRushPresentation(host,inspect,{onProgress=()=>{},
  ...
  try {
    ...
    const send=async(action,data={})=>{
      if(action==='codegen'){
        const{rushpresentation,...config}=data;
        if(typeof rushpresentation!=='boolean')throw Error('Replay timing mode missing');
        if(JSON.stringify(config)!==JSON.stringify(codegen))throw Error('Timing replay cannot change instruction configuration');
        const applied=await command('rushPresentation',{enabled:rushpresentation});
        if(applied.enabled!==rushpresentation)throw Error('Replay timing mode did not apply');
    ...
    result=await verify(send,inspect,{frames:600,
      codegenReference:{...codegen,rushpresentation:true},codegenComparison:{...codegen,rushpresentation:false},
      originalCodegen:{...codegen,rushpresentation:original},onProgress,
  ...
  const errors=[];
  for(const cleanup of[()=>command('pause'),()=>typeof original==='boolean'?command('rushPresentation',{enabled:original}):null,()=>host.adapter.request('start',{})]){
    try{await cleanup();}catch(error){errors.push(error.message);}
  }
  if(failed||errors.length)throw Error([failed?.message,...errors].filter(Boolean).join('; '));
  return{...result,kind:'normal-running-browser-rush-presentation-equivalence',timingTransitions:transitions};
}
